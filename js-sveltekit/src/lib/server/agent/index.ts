 ...
 *
 * This matches the Cloudflare Worker example: the coordinator delegates through
 * the built-in `task` tool, and delegated runs stream under subagent namespaces
 * surfaced by the frontend SDK.
 */
 ...
 *
 * It is process-local and volatile. On Cloudflare, isolate restarts and
 * multiple isolates can clear or split this state; swap in durable persistence
 * before treating the example as production-ready.
 */
...
/** The compiled agent type, re-exported for Svelte stream typing. */
